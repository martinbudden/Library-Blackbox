...
public:
    ...
    virtual void load_slow_state(blackbox_slow_state_t& slow_state, const blackbox_context_t& pg) = 0;

    //! Load the main state of the blackbox, used for I-frames and P-frames
    virtual void load_main_state(blackbox_main_state_t& mainState, uint32_t current_time_us, const blackbox_context_t& pg) = 0;
    virtual void load_gps_state(blackbox_gps_state_t& gps_state, const blackbox_context_t& pg) = 0;

    virtual bool is_armed(const blackbox_context_t& pg) const = 0;
    virtual bool is_blackbox_mode_active(const blackbox_context_t& pg) const = 0;
    virtual bool is_blackbox_erase_mode_active(const blackbox_context_t& pg) const = 0;
    virtual bool is_blackbox_mode_activation_condition_present(const blackbox_context_t& pg) const = 0;
    virtual uint32_t get_arming_beep_time_microseconds(const blackbox_context_t& pg) const = 0;
    virtual bool are_motors_running(const blackbox_context_t& pg) const = 0;
    virtual uint32_t rc_mode_activation_mask(const blackbox_context_t& pg) const = 0; // lower 32 bits of BOX_COUNT bits
    virtual void beep(const blackbox_context_t& pg) const = 0;
};
