...
BlackboxTask::BlackboxTask(uint32_t task_interval_microseconds, Blackbox& blackbox, MessageQueueBase& message_queue, blackbox_context_t& context) :
...
void BlackboxTask::loop()
{
    ...
    if (_time_microseconds_delta >= _task_interval_microseconds) { // if _taskIntervalMicroSeconds has passed, then run the update
        ...
        _blackbox.update_log(_context, time_microseconds);
    }
}
