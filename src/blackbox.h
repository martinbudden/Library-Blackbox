...
public:
    ...
    virtual write_e write_system_information(const blackbox_context_t& ctx) = 0;

    uint32_t update_log(const blackbox_context_t& ctx, uint32_t current_time_us); // main loop function, updates the blackbox log
    ...
    bool log_sframe_if_needed(const blackbox_context_t& ctx);
    ...
    void log_event_arming_beep_if_needed(const blackbox_context_t& ctx); // E-frame
    void log_event_flight_mode_if_needed(const blackbox_context_t& ctx); // E-frame

    void log_iteration(time_us_t current_time_us, const blackbox_context_t& ctx);
    ...
    bool in_motor_test_mode(const blackbox_context_t& ctx);
